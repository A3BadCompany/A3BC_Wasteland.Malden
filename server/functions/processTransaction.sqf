...
switch (toLower _type) do
{
	...
	case "atm":
	{
		...
		if (_amount != 0) then
		{
			...
			if (_newBalance > ["A3W_atmMaxBalance", 100000000] call getPublicVar) exitWith {}; // account would exceed or has reached max balance
	...
	case "atmtranfer":
	{
		...
		if (_total > 0) then
		{
			...
			if (_rBalance + _amount > ["A3W_atmMaxBalance", 100000000] call getPublicVar) exitWith {}; // recipient would exceed or has reached max balance
