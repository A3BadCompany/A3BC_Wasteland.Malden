...
if (!isNil "_itemEntry") then
{
	...
	if (count _itemEntry > 3) then
	{
		switch (_itemEntry select 3) do
		{
			case "backpack":
			{
				...
				switch (true) do
				{
					...
					{
						_description = "Remote-controlled hexacopter to bomb the shit out of 'em, pre-packaged in a backpack.<br/>UAV Terminal sold separately. Ages 8+<br/><t color='#f60606'>NO THERMAL IMAGING</t>";
					};
					case (_itemType isKindOf "UAV_06_medical_backpack_base_F"):
					{
						_description = "Remote-controlled hexacopter to revive and heal your teammates, pre-packaged in a backpack.<br/>UAV Terminal sold separately. Ages 8+<br/><t color='#f60606'>NO THERMAL IMAGING</t>";
					};
					case (_itemType isKindOf "UAV_06_backpack_base_F"):
					{
						_description = "Remote-controlled hexacopter to spy on your neighbors, pre-packaged in a backpack.<br/>UAV Terminal sold separately. Ages 8+<br/><t color='#f60606'>NO THERMAL IMAGING</t>";
					};
					case (["_UAV_01_backpack_", _itemType] call fn_findString != -1):
					{
						_description = "Remote-controlled quadcopter to spy on your neighbors, pre-packaged in a backpack.<br/>UAV Terminal sold separately. Ages 8+<br/><t color='#0cf204'>THERMAL IMAGING</t>";
					};
					case (["_Static_Designator_", _itemType] call fn_findString != -1):
					{
						_description = "Remote-controlled laser designator.<br/>UAV Terminal sold separately.<br/><t color='#f60606'>NO THERMAL IMAGING</t>";
					};
					default
					{
						//_name = _itemText;
		...
		if (_itemType isKindOf "UAV_06_backpack_base_F") then
		{
			...
			{
				_description = format ["%1%2%3", _description, ["<br/>",""] select (_description isEqualTo ""), "<t color='#f60606'>NO THERMAL IMAGING</t>"];
			};
		};
	};
};
