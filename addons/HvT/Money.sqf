...
while {true} do
{
	...
	{
		hint parseText ([
			"<t color='#FF0000' size='1.5' align='center'>High Value Target (MONEY)</t>",
			//profileName,
	...
	if (diag_tickTime - _lastMarker >= MARKER_REFRESH || (!alive _markerTarget && _hasMarker)) then
	{
		...
		if (_isHvT) then
		{
			...
			_markerName setMarkerText format ["High Value Target (MONEY): %1 ($%2k)", profileName, (floor ((player getVariable ["cmoney",0]) / 1000)) call fn_numToStr];
